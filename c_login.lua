...
local loginTitleText = "Khaled Bo Walid"
...
function LoginScreen_openLoginScreen(title)--local screenW, screenH = guiGetScreenSize()--local t1 = screenH * 0.0000--local t2 = screenH * 0.0000--local alpha = 0 --function startt()dxDrawRectangle(screenW * 0.0000, screenH * 0.0000, screenW * 1.0000, t1, tocolor(0, 0, 0, 255), false)dxDrawRectangle(screenW * 0.0000, screenH * 1.0000, screenW * 1.0000, t2, tocolor(0, 0, 0, 255), false)dxDrawImage(screenW * 0.1494, screenH * 0.3841, screenW * 0.6699, screenH * 0.2174, ":72o2/start.png", 0, 0, 0, tocolor(255, 255, 255, alpha), false)end--function startanim()t1 = t1 + 5if t1 > screenH * 0.3346 thent1 = screenH * 0.3346endt2 = t2 - 5if t2 < screenH * -0.3346 thent2 = screenH * -0.3346endif t1 == screenH * 0.3346 and t2 == screenH * -0.3346 thenalpha = alpha + 5 if alpha > 254 thenalpha = 255 endendendfunction stopanim()t1 = t1 - 5if t1 <= screenH * 0.0000 thent1 = screenH * 0.0000endt2 = t2 + 5if t2 >= screenH * 0.0000 thent2 = screenH * 0.0000endalpha = alpha - 5 if alpha < 1 thenalpha = 0 end  if alpha == 0 and t1 ==  screenH * 0.0000 thenremoveEventHandler("onClientRender", root, startt)endendfunction start ()addEventHandler("onClientRender", root, startt)addEventHandler("onClientRender", root, startanim)setTimer( function()addEventHandler("onClientRender", root, stopanim)removeEventHandler("onClientRender", root, startanim)end, 5000, 1)endstart()--setTimer( function()--
open_log_reg_pannel()--end, 9000, 1)--
	--[[
	guiSetInputEnabled(true)
	showCursor(true)
	if not title then
