...
function updateSetting(name, value)
	...
	if client and name and value then
		...
		if id then
			...
			local existed  = mysql:query_fetch_assoc("SELECT * FROM `account_settings` WHERE `name` = '"..name.."' AND `id` = '"..id.."' LIMIT 1") or false
			if existed and tonumber(existed.id) then 
				mysql:query_free("UPDATE `account_settings` SET `value` = '"..value.."' WHERE `id` = '"..id.."' AND `name` = '"..name.."' ")
			else
				mysql:query_free("INSERT INTO `account_settings` SET `name` = '"..name.."', `value` = '"..value.."', `id` = '"..id.."'")
			end
		end
	end
end
...
function updateCharacterSetting(name, value)
	...
	if client and name and value then
		...
		if id then
			name = mysql:escape_string(name)
			value = mysql:escape_string(value)
			local existed  = mysql:query_fetch_assoc("SELECT * FROM `character_settings` WHERE `name` = '"..name.."' AND `id` = '"..id.."' LIMIT 1") or false
			if existed and tonumber(existed.id) then 
				mysql:query_free("UPDATE `character_settings` SET `value` = '"..value.."' WHERE `id` = '"..id.."' AND `name` = '"..name.."' ")
			else
				mysql:query_free("INSERT INTO `character_settings` SET `name` = '"..name.."', `value` = '"..value.."', `id` = '"..id.."'")
			end
		end
	end
end
