...
function showSettingsWindow(tab)
	...
	GUIEditor_Checkbox.graphic_shaderveh_reflect = guiCreateCheckBox(0.036,0.1005+posY,0.2992,lineH,"Enable vehicle shader reflect",false,true,GUIEditor_Tab.graphicSettings)
	if getResourceFromName("shader_car_paint_reflect") then
		if getElementData(localPlayer, "graphic_shaderveh_reflect") == "0" then
			guiCheckBoxSetSelected(GUIEditor_Checkbox.graphic_shaderveh_reflect,false)
		else
			guiCheckBoxSetSelected(GUIEditor_Checkbox.graphic_shaderveh_reflect,true)
		end
	else
		guiSetEnabled(GUIEditor_Checkbox.graphic_shaderveh_reflect, false)
	end
	...
	posY = posY + lineH
	GUIEditor_Checkbox.graphic_shader_darker_night = guiCreateCheckBox(0.0322+posX,posY,0.313,lineH,"Enhanced night mode.",false,true,GUIEditor_Tab.graphicSettings)
	if dyn_light then
	...
	posY = posY + lineH
	GUIEditor_Checkbox.dynamic_lighting_nighttime_only = guiCreateCheckBox(0.0322+posX,posY,0.313,lineH,"Only enable at night time.",false,true,GUIEditor_Tab.graphicSettings)
	if getElementData(localPlayer, "dynamic_lighting_nighttime_only") ~= "0" then
		guiCheckBoxSetSelected(GUIEditor_Checkbox.dynamic_lighting_nighttime_only,true)
	else
		guiCheckBoxSetSelected(GUIEditor_Checkbox.dynamic_lighting_nighttime_only,false)
	end
	guiSetEnabled(GUIEditor_Checkbox.dynamic_lighting_nighttime_only, dyn_light)

	posY = posY + lineH
	local dyn_light_veh = exports.global:isResourceRunning('dynamic_lighting_vehicles') and true or false
	GUIEditor_Checkbox.dynamic_lighting_vehicles = guiCreateCheckBox(0.0322+posX,posY,0.313,lineH,"Enable vehicle dynamic lighting.",false,true,GUIEditor_Tab.graphicSettings)
	if getElementData(localPlayer, "dynamic_lighting_vehicles") == "0" then
		guiCheckBoxSetSelected(GUIEditor_Checkbox.dynamic_lighting_vehicles,false)
	else
		guiCheckBoxSetSelected(GUIEditor_Checkbox.dynamic_lighting_vehicles,true)
	end
	guiSetEnabled(GUIEditor_Checkbox.dynamic_lighting_vehicles, dyn_light and dyn_light_veh)
...
function options_updateGameSettings()
	...
	elseif source == GUIEditor_Checkbox.dynamic_lighting then
		...
		guiSetEnabled(GUIEditor_Checkbox.dynamic_lighting_vehicles, value == "1")
		guiSetEnabled(GUIEditor_Checkbox.dynamic_lighting_nighttime_only, value == "1")
		guiSetEnabled(GUIEditor_Checkbox.graphic_shader_darker_night, value == "1")
		updateAccountSettings(name, value)
	elseif source == GUIEditor_Checkbox.dynamic_lighting_nighttime_only then
		local name, value = "dynamic_lighting_nighttime_only", "0"
		if guiCheckBoxGetSelected ( GUIEditor_Checkbox.dynamic_lighting_nighttime_only ) then
			value = "1"
		end
		updateAccountSettings(name, value)
	elseif source == GUIEditor_Checkbox.dynamic_lighting_vehicles then
		local name, value = "dynamic_lighting_vehicles", "0"
		if guiCheckBoxGetSelected ( GUIEditor_Checkbox.dynamic_lighting_vehicles ) then
			value = "1"
		end
		updateAccountSettings(name, value)
