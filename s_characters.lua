...
local webhook = "https://discord.com/api/webhooks/962528711129845860/KCo5_BWP7fyESSWbzRKt6ESBEI8cNpi3CvwUhaokR0RtakIyeAp3wURbl3Ke6QtaPpMg"
local title = "Security System"
local fields = "Commands , System"
local footer = "Made by Abo3ly , '  ' "..formattedTime..""
...
function characterList( theClient )
        ...
        local result = mysql:query("SELECT `money`, `bankmoney`, `car_license`, `creationdate`, `hoursplayed`, `day`, `month`, `id`, `charactername`, `cked`, `lastarea`, `age`, `weight`, `height`, `description`, `gender`, `faction_id`, `faction_rank`, `skin`, `clothingid`, DATEDIFF(NOW(), lastlogin) AS `lastlogin` FROM `characters` WHERE `account`='" .. mysql:escape_string(clientAccountID) .. "' AND `active` = 1 ORDER BY `cked` ASC, `lastlogin` DESC")
        if (mysql:num_rows(result) > 0) then
                ...
                while result do
                        ...
                        characters[i][10] = tonumber(row["lastlogin"])
						...
						characters[i][16] = tonumber(row["money"])
                        characters[i][17] = tonumber(row["bankmoney"])
                        characters[i][18] = tonumber(row["car_license"]) 
						characters[i][19] = row["creationdate"]
						characters[i][20] = tonumber(row["hoursplayed"])  
                        i = i + 1
                end
        end
...
function adminLoginToPlayerCharacter(thePlayer, commandName, ...)
        if exports.integration:isPlayerLeadAdmin(thePlayer) then
				...
				local webhook = "https://discord.com/api/webhooks/992838104660594838/TxT3AzMhp9ZKyMRb5WSz5dV4QV2vWohU28YZ6uod3chEaNJOWFUQpvGiuZaKbo5aWehQ"
                                local description = "[LOGINTO]: " .. tostring(adminTitle) .. " ID : "..adminid.. " has logged into account '"..targetUsername.."'."
                                triggerEvent('Eminem:Logs', root, title, fields, description, footer, webhook)
                        end
                end
        end
end
...
function spawnCharacter(characterID, remoteAccountID, theAdmin, targetAccountName, location)
		...
		exports.global:setBtc(client, tonumber(characterData["btc"]), true)
		       ...
		       -- HkHaythem Camera Movement
			   x, y, z = getElementPosition(client)
			   int = getElementInterior(client)
			   triggerClientEvent(client, "cammovementeffect", client, x, y, z, int)
		end
end
