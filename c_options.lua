...
function options_showmenu()
	...
	local windowWidth, windowHeight = 250, 290
	local bHeight = 35
	...
	wOptions = exDGS:dgsCreateWindow(left, top, windowWidth, windowHeight, "=> F10 || By Eminem <=", false,_,_,_, tocolor(0, 0, 0, 255))
	exDGS:dgsSetFont ( wOptions, "default-bold" )
	exDGS:dgsWindowSetSizable(wOptions, false)
	exDGS:dgsWindowSetCloseButtonEnabled(wOptions, false)	
--exDGS


	bChangeCharacter = exDGS:dgsCreateButton(margin, margin, 230, 30, "تغير شخصية", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
	exDGS:dgsSetFont ( bChangeCharacter, "default-bold" )
	addEventHandler("onDgsMouseClickUp", bChangeCharacter,
		function ()
	...
	bStatistics = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "معلومات شخصية", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
	exDGS:dgsSetFont ( bStatistics, "default-bold" )
	addEventHandler("onDgsMouseClickUp", bStatistics,
	function ()
		if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	bGraphicsSettings = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "إعدادات", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
	exDGS:dgsSetFont ( bGraphicsSettings, "default-bold" )
	--addEventHandler("onClientGUIClick", bGraphicsSettings, options_opengraphicsmenu, false)
	addEventHandler("onDgsMouseClickUp", bGraphicsSettings,
	function ()
		if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	if getResourceFromName("donators") then
		bStore = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "ميزات مدفوعة", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
		exDGS:dgsSetFont ( bStore, "default-bold" )
		--addEventHandler("onClientGUIClick", bGraphicsSettings, options_opengraphicsmenu, false)
		addEventHandler("onDgsMouseClickUp", bStore,
		function ()
			if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	if getResourceFromName("admin-system") and exports['admin-system']:canPlayerAccessStaffManager(localPlayer) then
		bStaffManager = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "مسؤل الموظفين", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
		exDGS:dgsSetFont ( bStaffManager, "default-bold" )
		addEventHandler("onDgsMouseClickUp", bStaffManager,
		function ()
			if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	if getResourceFromName("interior-system") and getResourceFromName("interior-manager") and exports.integration:isPlayerAdmin( localPlayer ) then
		bInteriorManager = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "مسؤل البيوت", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
		exDGS:dgsSetFont ( bInteriorManager, "default-bold" )
		addEventHandler("onDgsMouseClickUp", bInteriorManager,
		function ()
			if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	if getResourceFromName("apps") and exports.integration:isPlayerTrialAdmin(localPlayer) or exports.integration:isPlayerSupporter(localPlayer) then
		bApplicationManager = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "مسؤل تطبيق", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
		exDGS:dgsSetFont ( bApplicationManager, "default-bold" )
		addEventHandler("onDgsMouseClickUp", bApplicationManager,
		function ()
			if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	if getResourceFromName("tele-manager") and exports.integration:isPlayerTrialAdmin(localPlayer) or exports.integration:isPlayerSupporter(localPlayer) then
		bTeleportManager = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "مسؤل الانتقال", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
		exDGS:dgsSetFont ( bTeleportManager, "default-bold" )
		addEventHandler("onDgsMouseClickUp", bTeleportManager,
		function ()
			if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	if getResourceFromName("carradio") then
		bRadioManager = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "مسؤل الراديو", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
		exDGS:dgsSetFont ( bRadioManager, "default-bold" )
		addEventHandler("onDgsMouseClickUp", bRadioManager,
		function ()
			if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	if getResourceFromName('vehicle') and getResourceFromName("vehicle_manager") then
		...
		if exports.integration:isPlayerVCTMember(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerVehicleConsultant(thePlayer) then
			bVehicleLib = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "قائمة سيارات", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
			exDGS:dgsSetFont ( bVehicleLib, "default-bold" )
			addEventHandler("onDgsMouseClickUp", bVehicleLib,
				function ()
					if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	if getResourceFromName("announcement") and exports.announcement:canPlayerAccessMotdManager(localPlayer) then
		bmotd = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "رسالة اليومية", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
		exDGS:dgsSetFont ( bmotd, "default-bold" )
		addEventHandler("onDgsMouseClickUp", bmotd,
			function ()
				if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	if getResourceFromName("help") then
		bHelp = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "مساعدة ومعلومات", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
		exDGS:dgsSetFont ( bHelp, "default-bold" )
		addEventHandler("onDgsMouseClickUp", bHelp,
			function ()
				if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	bLogout = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "تسجيل الخروج", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
	exDGS:dgsSetFont ( bLogout, "default-bold" )
	addEventHandler("onDgsMouseClickUp", bLogout,
		function ()
			if not isPedDead ( getLocalPlayer() ) and isCameraOnPlayer() then
	...
	bClose = exDGS:dgsCreateButton(margin, wHeight, 230, 30, "إغلاق", false, wOptions,false,false,false,false,false,false,0x9D9D00FF)
	exDGS:dgsSetFont ( bClose, "default-bold" )
	addEventHandler("onDgsMouseClickUp", bClose, options_closemenu, false)
	wHeight = wHeight + bHeight
